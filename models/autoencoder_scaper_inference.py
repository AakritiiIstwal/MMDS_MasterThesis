...
def main():
    ...
    model_path = Path('models/checkpoints/conv_autoencoder_150_50.pth')
    ...
    train_dir = Path('/work/aistwal/MasterThesis/data/jointSoundScene_data/syntheticSoundscenes/train_log_mel_features/features')
    encoded_feature_dir = Path('/work/aistwal/MasterThesis/data/jointSoundScene_data/syntheticSoundscenes/encoded_log_mel_features/features')
    decoded_feature_dir = Path('/work/aistwal/MasterThesis/data/jointSoundScene_data/syntheticSoundscenes/decoded_log_mel_features/features')
    ...
    for file in tqdm(train_dir.glob('*.pt')):
        try:
            ...
            input_data = torch.load(file).to(device)  # Ensure there's a batch dimension
...
if __name__=="__main__":
    ...
    encoder_df = main()
    df_save_path = "data/encoder_data/conv_autoencoder_150_50.pkl"
    encoder_df.to_pickle(df_save_path)
    print(f"--------SAVED DATAFRAME TO {df_save_path}-------")

    #READ PICKLE FILE
    unpickled_encoder_df = pd.read_pickle("data/encoder_data/conv_autoencoder_150_50.pkl")
