...
def main():
    ...
    parser.add_argument(
        "checkpoint", 
        help="The checkpoint from where the model has to be loaded"
    )
    parser.add_argument(
        "train_dir",
        help="The path where training features are saved"
    )
    parser.add_argument(
        "encoded_feature_dir",
        help="The path where encoded features need to be saved"
    )
    parser.add_argument(
        "decoded_feature_dir",
        help="The path where decoded features need to be saved"
    )
...
if __name__=="__main__":
    ...
    df_save_path = save_dir_path+"/tau_2019_conv_autoencoder_200_25.pkl"
    encoder_df.to_pickle(df_save_path)
    print(f"--------SAVED DATAFRAME TO {df_save_path}-------")

    #READ PICKLE FILE
    unpickled_encoder_df = pd.read_pickle(save_dir_path+"/tau_2019_conv_autoencoder_200_25.pkl")
